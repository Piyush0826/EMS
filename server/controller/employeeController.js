...
const completeProfile = async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!phone || !mobile || !address || !city || !state || !zipCode || !country) {
      return res.status(400).json({ 
        success: false, 
    ...
    if (employee.personalInfoCompleted) {
      return res.status(400).json({ 
        success: false, 
        error: "Personal information already completed" 
      });
    }

    // Update employee info and mark as completed
    const updatedEmployee = await Employee.findByIdAndUpdate(
      id,
      {
        dob,
    ...
    return res.status(500).json({ success: false, error: error.message || "complete profile server error" });
  }
};

export { addEmployee, upload, getEmployees, getEmployeeById, updateEmployee, getEmployeeByUserId, deleteEmployeesWithoutDept, completeProfile };
